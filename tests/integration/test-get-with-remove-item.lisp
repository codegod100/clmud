...
;; Try to define the get command with remove-item-from-room call
(format t "Testing get command with remove-item-from-room call...~%")

(handler-case
    (progn
      (define-command (("get6" "grab6") command-get6) (player rest)
      ...
      (format t "✓ Get command with remove-item-from-room defined successfully~%"))
    (error (err)
      (format t "✗ Error defining get command with remove-item-from-room: ~a~%" err)))

;; Check if it was registered
(let ((handler (gethash "get6" *command-dispatch*)))
  (if handler
      (format t "✓ Get6 command is registered: ~a~%" handler)
      (format t "✗ Get6 command is NOT registered~%")))
