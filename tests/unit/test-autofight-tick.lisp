...
;; Test autofight tick system
;; This test verifies that autofight works correctly with the tick system
...
(defun test-autofight-tick ()
  "Test that autofight works correctly with the tick system"
  ...
  (let* ((player (mud.player::make-player :name "TestPlayer" :room :village-square))
    ...
    (format t "~%=== Testing Autofight Tick System ===")
    ...
    ;; Test initial state - autofight should not trigger immediately
    (format t "~%~%Testing initial autofight check...")
    ...
    (setf (mud.mob::mob-last-autofight-time mob) 
          (- (get-universal-time) (1+ mud.constants::*tick-interval*)))
    
    ;; Test autofight after time has passed
    (format t "~%~%Testing autofight after time has passed...")
    (let ((*standard-output* (make-string-output-stream)))
      (mud.mob::process-autofight-actions mob))
    ...
    (format t "~%~%✅ Autofight tick test passed!")
    t))

(defun run-test ()
  "Run the autofight tick test"
  (handler-case
      (test-autofight-tick)
    (error (e)
