...
(defun save-game-state (&key (reason :periodic))
  (let ((path (state-file-path)))
    (with-mutex (*persistence-lock*)
      (handler-case
          (let ((count (mud.game-state:save-game-state :path path)))
            (when (not (eq reason :periodic))
              (server-log "Saved game state to ~a (~(~a~))" path reason))
            count)
        (error (err)
...
(defun load-server-game-state ()
  (let ((path (state-file-path)))
    (if (and path (probe-file path))
        (handler-case
            (mud.game-state::load-game-state path)
          (error (err)
