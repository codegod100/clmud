      ...
      (if (null inventory)
          ...
          (progn
            (let ((item-counts (make-hash-table :test 'equal))
                  ...
                  (equipped-armor (mud.player:player-equipped-armor player)))
              ;; Count items by name, but exclude equipped items
              (dolist (item inventory)
                (let ((item-name (mud.inventory::item-name item)))
                  ;; Only count items that are not currently equipped
                  (unless (or (and equipped-weapon (string= item-name (mud.inventory::item-name equipped-weapon)))
                              (and equipped-armor (string= item-name (mud.inventory::item-name equipped-armor))))
                    (incf (gethash item-name item-counts 0)))))
              
              ;; Calculate total count of non-equipped items
              (let ((total-count (loop for count being the hash-values of item-counts sum count)))
                (if (zerop total-count)
                    (format out "Your inventory is empty, but you carry ~d gold coins." (mud.player:player-gold player))
                    (progn
                      (format out "Inventory (~d item~:p):~%" total-count)
