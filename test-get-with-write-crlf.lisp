...
;; Try to define the get command with write-crlf call
(format t "Testing get command with write-crlf call...~%")

(handler-case
    (progn
      (define-command (("get7" "grab7") command-get7) (player rest)
      ...
      (format t "✓ Get command with write-crlf defined successfully~%"))
    (error (err)
      (format t "✗ Error defining get command with write-crlf: ~a~%" err)))

;; Check if it was registered
(let ((handler (gethash "get7" *command-dispatch*)))
  (if handler
      (format t "✓ Get7 command is registered: ~a~%" handler)
      (format t "✗ Get7 command is NOT registered~%")))
